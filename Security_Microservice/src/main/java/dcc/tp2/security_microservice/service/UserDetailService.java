...
public class UserDetailService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String combinedUsername) throws UsernameNotFoundException {
        ...
        if (userType.equals("ADMIN")){
            ...
            user_infos = restTemplate.getForObject("http://localhost:8092/User-Service/api/users/**"+email,HashMap.class);

        }

        if (userType.equals("USER")){

            user_infos = restTemplate.getForObject("http://localhost:8092/User-Service/api/users/{id}"+email,HashMap.class);
        }
